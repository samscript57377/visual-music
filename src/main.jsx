...
const DEFAULT_CUSTOM = {
  ringSize: 0.28, dotCount: 120, barHeight: 0.18,
  symmetry: 1, glow: 0.6, rotSpeed: 0.5, colorMix: 0.5,
};
...
function Solo3DVisualizer({ theme, metricsRef, soloShape, visible, showWire }) {
  ...
  useEffect(()=>{
    ...
    const render=()=>{
      ...
      const t=e.clock.getElapsedTime(),v=metricsRef.current,active=v.active,pulse=(v.beat*0.5+v.level*0.3)*active;
      ...
      e.mesh.rotation.y=t*(0.22+v.mids*0.3*active); e.mesh.rotation.x=Math.sin(t*0.4)*0.3+v.highs*0.2*active; e.mesh.rotation.z=Math.sin(t*0.28)*0.15;
      ...
      e.camera.position.x=Math.sin(t*0.18)*0.3*active; e.camera.position.y=Math.cos(t*0.14)*0.2*active; e.camera.lookAt(0,0,0);
    ...
    <div className="custom-controls">
      {CUSTOM_SLIDERS.map(sl=>(
        <div className="custom-slider-row" key={sl.key}>
          <div className="custom-slider-head"><span className="custom-slider-label">{sl.label}</span><span className="custom-slider-val">{sl.fmt(customParams[sl.key])}</span></div>
          <input type="range" min={sl.min} max={sl.max} step={sl.step} value={customParams[sl.key]} onChange={e=>onChangeParam(sl.key,parseFloat(e.target.value))} className="custom-range"/>
        </div>
      ))}
    </div>
  );
...
function SettingsScreen({ visType, setVisType, themeKey, setThemeKey, soloShape, setSoloShape, customParams, onChangeParam, options, setOption, customThemes, setCustomThemes, onClose }) {
  ...
  const addCustomTheme=()=>{
    const ct=newCustomTheme();
    setCustomThemes(prev=>[...prev,ct]);
    setThemeKey(ct.id);
  };

  const updateCustomTheme=(id,updated)=>{
    setCustomThemes(prev=>prev.map(ct=>ct.id===id?{...updated,id}:ct));
  };

  const deleteCustomTheme=(id)=>{
    setCustomThemes(prev=>prev.filter(ct=>ct.id!==id));
    if(themeKey===id) setThemeKey("io");
  };
        ...
        {tab==="theme"&&(<>
          ...
          <div className="settings-section">
            ...
            {customThemes.map(ct=>(
              <div key={ct.id}>
                <button
                  className={`cte-select-btn${themeKey===ct.id?" cte-select-btn--active":""}`}
                  onClick={()=>setThemeKey(ct.id)}
                  style={{"--t-accent":ct.accent}}
                >
...
function App(){
  ...
  return(
    <main ref={appRef} className={appClass}
      ...
      {options.bg3d&&<VisualStage theme={theme} metricsRef={renderMetricsRef} showParticles={options.bg3dParticles}/>}
      ...
      <Solo3DVisualizer theme={theme} metricsRef={renderMetricsRef} soloShape={soloShape} visible={isSolo} showWire={options.solo3dWire}/>
