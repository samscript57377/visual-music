...
import {
  ...
  CheckCircle2, Plus, Trash2, Palette, Link, Code, Share2
} from "lucide-react";
  ...
  return (
    <div className="share-field">
      ...
      <div className="share-field-row">
        <input className="share-field-input" readOnly value={value} onFocus={e=>e.target.select()}/>
        <button className="share-copy-btn" onClick={copy} aria-label="Copy">
          {copied ? <CheckCircle2 size={15}/> : <Link size={15}/>}
          {copied ? "Copied!" : "Copy"}
        </button>
      </div>
    </div>
      ...
      <div className="settings-section">
        ...
        <div className="share-embed-title-row">
          ...
          <input
            ...
            value={embedTitle}
            maxLength={40}
            placeholder="Visual Music"
            onChange={e => setEmbedTitle(e.target.value)}
          />
        </div>
        <CopyField label="Embed URL" value={embedUrl}/>
        <CopyField label="iframe snippet" value={iframeSnippet}/>
      </div>
...
function SettingsScreen({ visType, setVisType, themeKey, setThemeKey, soloShape, setSoloShape, customParams, onChangeParam, options, setOption, customThemes, setCustomThemes, sceneParams, onChangeSceneParam, shareUrl, embedUrl, embedTitle, setEmbedTitle, onClose }) {
  ...
  return (
    <div className="settings-overlay" role="dialog" aria-label="Settings">
      <div className="settings-panel">
        ...
        {tab==="visualiser"&&(<>
          <div className="settings-section">
            ...
            <div className="vis-grid">
              {VIS_TYPES.map(vt=>{const Icon=vt.icon,isActive=visType===vt.id;return(
        ...
        {tab==="theme"&&(<>
          <div className="settings-section">
            ...
            <div className="theme-grid-settings">
              {Object.entries(BUILTIN_THEMES).map(([key,item])=>{const isActive=key===themeKey;return(
        ...
        {tab==="share"&&(
          <SharePanel shareUrl={shareUrl} embedUrl={embedUrl} embedTitle={embedTitle} setEmbedTitle={setEmbedTitle}/>
        )}
      </div>
    </div>
  );
...
// ─── Embed bar (minimal UI for ?embed=1 mode) ─────────────────────────────────

function EmbedBar({ audioState, loadFile, toggle, embedTitle }) {
  ...
  const onDrop = e => {
    ...
    if (file && (file.type.startsWith("audio/") || file.name.endsWith(".mp3"))) loadFile(file);
  };

  return (
    <div
      className={`embed-bar${dropping?" embed-bar--dropping":""}`}
      onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
    >
      <span className="embed-bar-title">
        <Sparkles size={14}/>
        {embedTitle || "Visual Music"}
      </span>
      <span className="embed-bar-track">{audioState.name || (dropping ? "Drop to load…" : "Drop an MP3 or upload")}</span>
      <div className="embed-bar-actions">
        <label className="embed-upload-btn" title="Upload MP3">
          <Upload size={15}/>
          <input type="file" accept=".mp3,audio/mpeg" onChange={e=>loadFile(e.target.files?.[0])} style={{position:"absolute",width:1,height:1,opacity:0}}/>
        </label>
        <button className="embed-play-btn" onClick={toggle} disabled={!audioState.ready}>
          {audioState.playing ? <Pause size={16}/> : <Play size={16}/>}
        </button>
      </div>
      {/* track progress */}
      {audioState.ready && <div className="embed-bar-progress" style={{width:`${audioState.progress*100}%`}}/>}
    </div>
  );
}
...
function App(){
  // Parse URL params once — used to seed initial state
  const initParams = useMemo(() => parseEmbedParams(), []);
  ...
  const [visType,setVisType]=useState(()=>{
    if(initParams.vis && VIS_TYPES.find(v=>v.id===initParams.vis)) return initParams.vis;
    return "cosmic";
  });
    ...
    <main ref={appRef} className={appClass}
      ...
      {showSettings&&(
        <SettingsScreen
          ...
          shareUrl={shareUrl} embedUrl={embedUrl}
      ...
      {/* Embed mode: minimal floating bar instead of full control panel */}
      {isEmbed ? (
        <EmbedBar audioState={audioState} loadFile={loadFile} toggle={toggle} embedTitle={embedTitle||initParams.title}/>
      ) : (
        <section
          className={[
            "control-panel",
